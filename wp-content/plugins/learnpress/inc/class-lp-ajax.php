	...
	class LP_AJAX {
		...
		public static function _request_login() {
			...
			if ( $error ) {
				...
				$return['message'] = learn_press_get_message( sprintf( __( 'Bem-vindo de volta, %s! Redirecionando...', 'learnpress' ), learn_press_get_profile_display_name( $user ) ) );
			}
			...
			if ( !wp_verify_nonce( $security, sprintf( 'complete-item-%d-%d-%d', $user->id, $course_id, $id ) ) ) {
				...
				if ( $type == 'lp_lesson' ) {
					...
					if ( is_wp_error( $results ) ) {
						...
						learn_press_add_message( __( 'Você completou essa aula!', 'learnpress' ) );
					}
				} else {
		...
		public static function complete_lesson() {
			...
			if ( $response['result'] == 'success' ) {
				...
				if ( !is_wp_error( $result ) ) {
					...
					$response['button_text']   = '<span class="dashicons dashicons-yes"></span>' . __( 'Aula assistida!', 'learnpress' );
					...
					if ( $can_finish ) {
						...
						learn_press_display_message( __( 'Parabéns! Você terminou essa aula!', 'learnpress' ) );
					}
